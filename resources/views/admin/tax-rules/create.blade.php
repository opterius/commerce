<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-lg font-semibold text-gray-800">{{ __('tax_rules.create') }}</h2>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm p-6">
            <form method="POST" action="{{ route('admin.tax-rules.store') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <select id="applies_to" name="applies_to"
                        ...
                        @foreach (['all', 'hosting', 'one_time'] as $opt)
                            <option value="{{ $opt }}" @selected(old('applies_to', 'all') === $opt)>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <x-button type="submit">{{ __('tax_rules.create') }}</x-button>
                </div>
            </form>
        </div>
    </div>
