<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-lg font-semibold text-gray-800">{{ __('faq.create') }}</h2>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.faqs.store') }}" class="bg-white rounded-xl shadow-sm p-6 space-y-5">
            @csrf

            <div>
                <label class="form-label">{{ __('faq.question') }} <span class="text-red-500">*</span></label>
                <textarea name="question" rows="2" class="form-input" maxlength="500" required>{{ old('question') }}</textarea>
            </div>
