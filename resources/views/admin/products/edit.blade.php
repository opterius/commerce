<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-lg font-semibold text-gray-800">{{ __('products.edit_product') }}</h2>
        </div>
    </x-slot>
    ...
    <form method="POST" action="{{ route('admin.products.update', $product) }}">
        ...
        <div x-data="{ tab: 'details' }">
            ...
            <div x-show="tab === 'details'" x-cloak>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <div class="space-y-4">
                            <x-input name="name" :label="__('products.product_name')" :value="old('name', $product->name)" required />

                            <div>
                                <x-input name="slug" :label="__('products.product_slug')" :value="old('slug', $product->slug)" />
                                <p class="mt-1 text-xs text-gray-500">{{ __('products.slug_help') }}</p>
                            </div>

                            <x-textarea name="description" :label="__('products.product_description')" :value="old('description', $product->description)" rows="4" />
                    ...
                    {{-- Right column --}}
                    <div class="bg-white rounded-xl shadow-sm p-6" x-data="{ stockControl: {{ old('stock_control', $product->stock_control) ? 'true' : 'false' }} }">
                        <h3 class="text-base font-semibold text-gray-800 mb-4">{{ __('common.general') }}</h3>
                        <div class="space-y-4">
                            <x-select name="product_group_id" :label="__('products.product_group')" :options="$groupOptions" :selected="old('product_group_id', $product->product_group_id)" />
                            ...
                            ]" :selected="old('provisioning_module', $product->provisioning_module)" />
                            ...
                            <div>
                                ...
                                <select name="server_group_id" class="form-input">
                                    ...
                                    @foreach ($serverGroups as $sg)
                                        <option value="{{ $sg->id }}" {{ old('server_group_id', $product->server_group_id) == $sg->id ? 'selected' : '' }}>
                                            {{ $sg->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <x-input name="provisioning_package" :label="__('provisioning.provisioning_package')" :value="old('provisioning_package', $product->provisioning_package)" />
                            ...
                            <div>
                                <label class="inline-flex items-center gap-2">
                                    <input
                                        ...
                                        {{ old('stock_control', $product->stock_control) ? 'checked' : '' }}
                            ...
                            <div x-show="stockControl" x-cloak>
                                <x-input name="qty_in_stock" type="number" :label="__('products.qty_in_stock')" :value="old('qty_in_stock', $product->qty_in_stock)" />
                            </div>

                            <x-checkbox name="require_domain" value="1" :label="__('products.require_domain')" :checked="old('require_domain', $product->require_domain)" />

                            <x-input name="sort_order" type="number" :label="__('common.sort_order')" :value="old('sort_order', $product->sort_order)" />
                        </div>
                    </div>
                </div>
            </div>
                                    ...
                                    @foreach ($cycles as $cycle)
                                        @php
                                            $existing = $product->pricing
                                                ...
                                                ->first();
                                            $existingPrice = $existing && $existing->price !== null ? number_format($existing->price / 100, 2, '.', '') : '';
                                            $existingSetup = $existing && $existing->setup_fee !== null ? number_format($existing->setup_fee / 100, 2, '.', '') : '';
                                        @endphp
                                        <tr>
                                            ...
                                            <td class="px-6 py-3 whitespace-nowrap">
                                                <input
                                                    ...
                                                    value="{{ old('pricing.' . $currency->code . '.' . $cycle . '.price', $existingPrice) }}"
                                                    placeholder="0.00"
                                            ...
                                            <td class="px-6 py-3 whitespace-nowrap">
                                                <input
                                                    ...
                                                    value="{{ old('pricing.' . $currency->code . '.' . $cycle . '.setup_fee', $existingSetup) }}"
                                                    placeholder="0.00"
                                                    class="block w-32 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                                                />
                                            </td>
            ...
            @if (!empty($optionGroups) && count($optionGroups))
                @php
                    $selectedOptionGroups = old('option_groups', $product->configurableOptionGroups->pluck('id')->toArray());
                @endphp
                        ...
                        @foreach ($optionGroups as $optionGroup)
                            <label class="inline-flex items-center gap-2">
                                <input
                                    ...
                                    {{ in_array($optionGroup->id, $selectedOptionGroups) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                />
