<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-lg font-semibold text-gray-800">{{ __('tickets.create_canned_response') }}</h2>
        </div>
    </x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.canned-responses.store') }}" class="bg-white rounded-xl shadow-sm p-6 space-y-5">
            @csrf

            <x-input name="title" :label="__('tickets.response_title')" :value="old('title')" required />
            ...
            <div>
                ...
                <textarea name="body" rows="10" class="form-input font-mono text-sm" required>{{ old('body') }}</textarea>
