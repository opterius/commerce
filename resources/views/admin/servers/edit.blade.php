...
<div class="max-w-2xl space-y-6">
    ...
    <form method="POST" action="{{ route('admin.servers.update', $server) }}" class="card p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <x-input name="api_url" :label="__('provisioning.api_url')" :value="old('api_url', $server->api_url)" placeholder="https://panel.example.com" required />

        <div>
            <x-input name="api_token" :label="__('provisioning.api_token')" :value="''" type="password" />
            <p class="mt-1 text-xs text-gray-400">{{ __('provisioning.leave_blank_to_keep') }}</p>
        </div>
