...
@section('title', __('provisioning.create_server_group'))

@section('content')
<div class="max-w-xl space-y-6">
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.server-groups.index') }}" class="text-gray-400 hover:text-gray-600">←</a>
        <h1 class="text-2xl font-semibold text-gray-900">{{ __('provisioning.create_server_group') }}</h1>
    </div>

    <form method="POST" action="{{ route('admin.server-groups.store') }}" class="card p-6 space-y-5">
        @csrf
