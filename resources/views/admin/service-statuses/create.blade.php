<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-lg font-semibold text-gray-800">{{ __('announcements.add_component') }}</h2>
        </div>
    </x-slot>

    <div class="max-w-xl">
        <form method="POST" action="{{ route('admin.service-statuses.store') }}" class="bg-white rounded-xl shadow-sm p-6 space-y-5">
            @csrf

            <x-input name="name" :label="__('common.name')" :value="old('name')" required placeholder="Web Hosting" />
            ...
            <div>
                ...
                <select name="status" class="form-input">
                    @foreach (\App\Models\ServiceStatus::STATUSES as $s)
                        <option value="{{ $s }}" {{ old('status', 'operational') === $s ? 'selected' : '' }}>{{ __('announcements.status_' . $s) }}</option>
                    @endforeach
                </select>
            </div>
