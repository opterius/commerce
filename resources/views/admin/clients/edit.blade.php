<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-lg font-semibold text-gray-800">{{ __('clients.edit_client') }}</h2>
        </div>
    </x-slot>
    ...
    <form method="POST" action="{{ route('admin.clients.update', $client) }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <x-input name="tax_id" :label="__('clients.tax_id')" :value="old('tax_id', $client->tax_id)" />
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        <x-select name="currency_code" :label="__('clients.currency')" :options="$currencies ?? []" :selected="old('currency_code', $client->currency_code)" />
                        <x-select name="language" :label="__('clients.language')" :options="$languages ?? []" :selected="old('language', $client->language)" />
                        <x-select name="client_group_id" :label="__('clients.group')" :options="$groupOptions ?? []" :selected="old('client_group_id', $client->client_group_id)" />
                        ...
                        ]" :selected="old('status', $client->status)" />
                    </div>
                </div>
            </div>
        </div>
        ...
        @if (!empty($tags) && count($tags))
            @php
                $selectedTags = old('tags', $client->tags->pluck('id')->toArray());
            @endphp
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 mt-6">
                ...
                <div class="space-y-4">
                    @foreach ($customFields as $field)
                        @php
                            $fieldValue = old('custom_fields.' . $field->id, $customFieldValues[$field->id] ?? '');
                        @endphp
