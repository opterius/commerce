    ...
    <div class="max-w-2xl space-y-6">
        ...
        @if (isset($sessionSecret) && $sessionSecret)
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <form method="POST" action="{{ route('client.two-factor.confirm') }}" class="flex gap-3">
                    @csrf
                    <input
                        name="code"
                        type="text"
        ...
        @elseif ($client->two_factor_confirmed_at)
            {{-- 2FA is enabled --}}
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex items-center gap-3 mb-4">
                    <x-badge color="green">2FA is enabled</x-badge>
                    <span class="text-sm text-gray-500">
                        Activated {{ $client->two_factor_confirmed_at->format('M d, Y') }}
                    </span>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6" x-data="{ open: false }">
                ...
                <div x-show="open" x-cloak class="mt-4">
                    <form method="POST" action="{{ route('client.two-factor.recovery-codes') }}" class="flex gap-3 items-center">
                        @csrf
                        <input
                            name="password"
                            type="password"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-red-100 p-6" x-data="{ open: false }">
                <h3 class="text-base font-semibold text-red-700 mb-1">Disable Two-Factor Authentication</h3>
                <p class="text-sm text-gray-600 mb-4">
                ...
                <div x-show="open" x-cloak class="mt-4">
                    <form method="POST" action="{{ route('client.two-factor.disable') }}" class="flex gap-3 items-center">
                        @csrf
                        <input
                            name="password"
                            type="password"
        ...
        @else
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <form method="POST" action="{{ route('client.two-factor.enable') }}">
                    @csrf
                    <button type="submit"
                        class="px-4 py-2 rounded-md bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 transition">
                        Enable 2FA
